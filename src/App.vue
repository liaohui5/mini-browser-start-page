<template>
  <!-- windmill icon -->
  <img
    class="w-10 fixed right-12 top-12 transition-transform duration-1000 hover:cursor-pointer"
    :src="windmillIcon"
    ref="refreshBgRef"
  ...
  <!-- main content -->
  <div
    class="mx-auto flex justify-center items-center h-12 mt-32 bg-white rounded relative opacity-85 w-3/4 lg:w-1/3"
  >
    <!-- search enginer icon -->
    ...
    <div class="w-full absolute top-14 rounded bg-white p-4 flex flex-wrap" v-show="enginerVisible">
      ...
      >
        <img class="w-6 mr-5" :src="item.icon" /><span class="text-gray-500 flex-1">{{
          item.name
        }}</span>
      </div>
    </div>
...
const { enginer, enginerVisible, keyword, search, toggleEnginerVisible, changeSearchEnginer } =
  useSearchEnginer();
